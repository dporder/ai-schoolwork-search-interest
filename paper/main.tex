...
\author[1]{Dan Porder}
\affil[1]{University of London, \texttt{danporder@gmail.com},
ORCID \url{https://orcid.org/0009-0006-4680-011X}}
...
County records from ACS and SEDA are aggregated to the DMA level using a publicly available FIPS-to-DMA crosswalk~\citep{kapastor2020}. For rates we compute population-weighted means, and for counts we sum. After merging with the Google Trends scores, the full analytic frame contains 209 DMAs with complete predictor information, of which 198 have observed search-interest outcomes. The primary models use those 198; the remaining 11 enter only through the zero-coded sample reported alongside. Table~\ref{tab:vars} in Appendix~\ref{sec:appdata} lists all variables.
...
\paragraph{Google Trends reliability.} Google Trends results, however, are nondeterministic, as it does not count searches. Instead, it estimates their relative frequency from a randomly drawn sample of all Google queries, and because a fresh sample is drawn for each request, identical queries issued at different times can return different values~\citep{holzl2025, googletrends}. Normalization, integer rounding, and threshold suppression then amplify this sampling error precisely for the low-volume terms and small geographies used in this study, so our scores are reproducible only as of the single pull date recorded in the code release. The anchor-term strategy improves comparability across keyword batches but cannot eliminate all measurement error.
...
Code, processed data, and the reproducible analysis notebook are available at \url{https://github.com/dporder/ai-schoolwork-search-interest} and archived at \href{https://doi.org/10.5281/zenodo.21764028}{doi:10.5281/zenodo.21764028}. Raw Google Trends responses are provided as CSVs; Trends queries are not guaranteed to replicate exactly over time because the underlying API is non-deterministic, estimating search interest from a fresh random sample on each request.
