...
\large Socioeconomic and Educational Predictors of Student Interest in the ChatGPT Era}

\author[1]{Daniel Clopton Porder}
...
We find the latter. The result holds across multiple methods (OLS, Lasso, bootstrap Lasso, an unsupervised k-means typology, supervised gradient boosting regression). Regions that in 2019 were smaller, poorer, less educated, and had higher Black or Hispanic population shares show the highest 2023--2025 student AI-for-schoolwork search interest. Seven 2019 features explain about 42\% of the cross-region variance in that interest under 5-fold cross-validation.
...
We evaluate Linear, Ridge, Lasso, Random Forest, and Gradient Boosting under 5-fold cross-validation with \texttt{random\_state=42}. Linear, Ridge, and Lasso converge to roughly equivalent performance ($R^2 \approx 0.16$); Random Forest and Gradient Boosting both produce a substantially higher $R^2 \approx 0.39$, with Gradient Boosting marginally ahead of Random Forest after hyperparameter tuning. We therefore select tuned Gradient Boosting as the headline model, and use a grid search over \texttt{n\_estimators}, \texttt{learning\_rate}, \texttt{max\_depth}, and \texttt{min\_samples\_split} (with 5-fold CV inside the search) to fix its hyperparameters, then report mean $\pm$ SD of $R^2$ across an outer 5-fold cross-validation. The full comparison is reported in Section~\ref{sec:supervised}; the linear and tree-ensemble families agree qualitatively on which features matter most, but disagree by more than a factor of two on $R^2$, which is the basis for selecting the tree-ensemble family for the headline. Poverty rate, graduate/professional rate, and \texttt{pct\_nh\_white} are retained for sensitivity checks but excluded from the headline model to reduce redundancy.
...
\midrule
0 (rural \& small-metro)         & 102 & \$53{,}587 & 24.8\% & \phantom{0}1.5\% & \phantom{0,}741{,}215 & 41.7 \\
1 (Hispanic SW metros)           & \phantom{0}20 & \$55{,}049 & 23.1\% & \phantom{0}3.1\% & 2{,}070{,}279 & 40.9 \\
2 (large national metros)        & \phantom{0}56 & \$68{,}095 & 35.0\% & \phantom{0}4.4\% & 3{,}133{,}189 & 18.7 \\
...
We compared five regressors on the seven-feature supervised predictor set using 5-fold cross-validation (Table~\ref{tab:models}). Tree ensembles clearly outperform the linear family. Under 5-fold CV with grid-searched hyperparameters and \texttt{random\_state=42}, the tuned gradient boosting model achieves mean $R^2 = 0.42 \pm 0.12$ (SD across folds) and a mean absolute error of $\approx 14.2$ search-interest points. A single 80/20 split estimate of $R^2 = 0.547$ provides a useful upper-end reference, but at $n=209$ a single split is too sensitive to fold composition, and the cross-validated value is the appropriate honest estimate.

We take the CV-based $R^2 \approx 0.42$ as the headline. It is substantially lower than the single-split estimate, but it still comfortably exceeds the $R^2 \approx 0.16$ ceiling of any linear model on the same features, and it is respectable given the small sample ($n=209$) and the measurement limits of both Google Trends and county-level ACS/SEDA aggregates. The qualitative story---that pre-existing disadvantage predicts subsequent student AI-for-schoolwork interest---is unchanged.
...
\midrule
Random Forest             & 0.388 & 0.073 & 14.7 \\
Gradient Boost (tuned)    & 0.417 & 0.122 & 14.2 \\
Gradient Boost (default)  & 0.380 & 0.149 & 14.7 \\
Lasso                     & 0.160 & 0.056 & 20.2 \\
Ridge                     & 0.157 & 0.065 & 20.2 \\
Linear                    & 0.157 & 0.066 & 20.2 \\
\bottomrule
\end{tabular}
\caption{Cross-validated performance (5 folds, \texttt{random\_state=42}) of five regressors on the final seven-feature supervised predictor set (\texttt{median\_income}, \texttt{bach\_plus\_rate}, \texttt{population}, \texttt{pct\_hispanic}, \texttt{pct\_nh\_black}, \texttt{pct\_nh\_asian}, \texttt{score\_all\_ela}). The tuned gradient boosting model (grid search over \texttt{n\_estimators} $\in\{100,200,300\}$, \texttt{learning\_rate} $\in\{0.01,0.1,0.2\}$, \texttt{max\_depth} $\in\{3,5,7\}$, \texttt{min\_samples\_split} $\in\{2,5\}$) is the reported headline model.}
\label{tab:models}
\end{table}
...
We asked what pre-ChatGPT regional conditions predict 2023--2025 student-intent search interest in AI for schoolwork. The answer is: smaller, lower-income, less-educated DMAs with higher shares of Black and Hispanic residents, which is an inversion of the usual ``technology adoption tracks advantage'' story and a divergence from the general-purpose ChatGPT search divide documented in prior regional work. A gradient-boosting model on seven 2019 features explains about 42\% of the cross-region variance in post-ChatGPT student AI-for-schoolwork search interest under 5-fold cross-validation. The result aligns with newer student-level survey evidence showing elevated schoolwork-related chatbot use among Black and Hispanic teens, but it adds a distinct regional signal: places with smaller populations, lower income, and lower educational attainment show greater search interest in AI for schoolwork. The findings do not identify why, but they narrow the set of plausible explanations, from which we highlight three (access-seeking, disengagement, and non-student traffic), all of which have the same policy implication. That is, we should treat student AI interest as a signal of resource gaps, not as misconduct to be policed, and resist blanket punitive responses that will fall hardest on the communities most motivated to use the tools.
...
Code, processed data, and the reproducible analysis notebook are available at \url{https://github.com/danporder/ai-schoolwork-search-interest} and archived at \href{https://doi.org/10.5281/zenodo.20185969}{doi:10.5281/zenodo.20185969}. Raw Google Trends responses are provided as CSVs; Trends queries are not guaranteed to replicate exactly over time because the underlying API is non-deterministic.
...
Feature & VIF \\
\midrule
\texttt{median\_income}    & 4.18 \\
\texttt{bach\_plus\_rate}  & 3.31 \\
\texttt{pct\_nh\_asian}    & 1.82 \\
\texttt{population}        & 1.67 \\
\texttt{score\_all\_ela}   & 1.48 \\
\texttt{pct\_nh\_black}    & 1.47 \\
\texttt{pct\_hispanic}     & 1.24 \\
\bottomrule
\end{tabular}
\caption{Variance inflation factors for the seven supervised features (z-scored, $n=209$). All values are below the conventional VIF threshold of 5.}
\label{tab:vif}
\end{table}
